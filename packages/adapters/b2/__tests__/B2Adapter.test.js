 ...
 */
const B2Adapter_1 = require("../src/B2Adapter");
...
function makeAdapter(responses) {
    return new B2Adapter_1.B2Adapter({
...
describe("B2Adapter.initTransfer", () => {
    ...
    it("caches auth token across calls", async () => {
        ...
        const adapter = new B2Adapter_1.B2Adapter({
            applicationKeyId: "k",
            applicationKey: "s",
            bucketId: "b",
            fetch: fetchMock,
    ...
    it("throws authError on 401 during authorisation", async () => {
        ...
        await expect(adapter.initTransfer(makeSession())).rejects.toBeInstanceOf(core_1.TransferError);
    });
});
// ── uploadChunk ────────────────────────────────────────────────────────────────
describe("B2Adapter.uploadChunk", () => {
    it("gets upload-part URL then uploads data, returns providerToken", async () => {
        ...
        const chunk = (0, core_1.makeChunkMeta)(0, 0, 1024);
        const result = await adapter.uploadChunk(session, chunk, new Uint8Array(1024), "sha256fake");
    ...
    it("throws if providerSessionId is missing", async () => {
        ...
        await expect(adapter.uploadChunk(session, (0, core_1.makeChunkMeta)(0, 0, 512), new Uint8Array(512), "sha")).rejects.toBeInstanceOf(core_1.TransferError);
    });
    ...
    it("throws rateLimit error on HTTP 429", async () => {
        ...
        const err = await adapter
            .uploadChunk(session, (0, core_1.makeChunkMeta)(0, 0, 512), new Uint8Array(512), "sha")
            .catch((e) => e);
        expect(err).toBeInstanceOf(core_1.TransferError);
...
describe("B2Adapter.completeTransfer", () => {
    it("calls b2_finish_large_file with sorted sha1 tokens", async () => {
        ...
        const chunks = [
            {
                ...(0, core_1.makeChunkMeta)(1, 1024, 1024),
                status: "done",
                providerToken: "sha1-b",
            },
            {
                ...(0, core_1.makeChunkMeta)(0, 0, 1024),
                status: "done",
...
//# sourceMappingURL=B2Adapter.test.js.map